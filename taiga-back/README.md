...
This Docker image can be used for running the Taiga backend. It works together with the [bhromor/taiga-front-dist](https://registry.hub.docker.com/u/bhromor/taiga-front-dist/) image.
...
```
docker run --name taiga_back_container_name --link postgres_container_name:postgres bhromor/taiga-back
```

## Docker-compose

For a complete taiga installation (``bhromor/taiga-back`` and ``bhromor/taiga-front-dist``) you can use this docker-compose configuration:
...
* ``API_SCHEME`` defaults to ``"http"``. Use ``https`` if ``bhromor/taiga-front-dist`` is used and SSL enabled.
* ``API_DOMAIN`` defaults to ``"$HOSTNAME"``
* ``API_NAME`` defaults to ``"api"``
* ``FRONT_SCHEME`` defaults to ``"http"``. Use ``https`` if ``bhromor/taiga-front-dist`` is used and SSL enabled.
